...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	struct Env* myenv = curenv; //The calling environment
		...
		int share_obj = get_share_object_ID(ownerID , shareName);
		if(share_obj != E_SHARED_MEM_NOT_EXISTS)
		      return E_SHARED_MEM_EXISTS ;
		struct Share *alloc_obj = NULL ;
		int obj_id = allocate_share_object(&alloc_obj);
		if(obj_id == E_NO_SHARE)
		{
			return E_NO_SHARE ;
		}

		uint32 v_a = ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
		uint32 size2 = ROUNDUP(size + v_a,PAGE_SIZE);
		int index = 0 ;
		for(uint32 i = v_a ; i < size2 ; i += PAGE_SIZE)
		{
	      ...
	      int ret1 = map_frame(myenv->env_page_directory,frame_info,i,PERM_WRITEABLE | PERM_USER);
	      if(ret1 == E_NO_MEM)
	              {
	      	       return E_NO_MEM;
	             }

				add_frame_to_storage(alloc_obj->framesStorage , frame_info , index );
		        index++ ;
		}

		alloc_obj->ownerID = ownerID ;
		alloc_obj->size = size ;
		strcpy(alloc_obj->name, shareName);
	    alloc_obj->references = 1 ;
	    alloc_obj->isWritable = isWritable ;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
	   ...
	   if(obj_id == E_SHARED_MEM_NOT_EXISTS)
	   	      return E_SHARED_MEM_NOT_EXISTS ;
	   uint32 v_a = ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
	   uint32 size2 = ROUNDUP(shares[obj_id].size + v_a,PAGE_SIZE);
	   int index = 0 ;
	   for(uint32 i = v_a ; i < size2 ; i += PAGE_SIZE)
	   	{
	   struct FrameInfo *frameinfo = NULL ;
	   frameinfo = get_frame_from_storage(shares[obj_id].framesStorage,index);
	   index++ ;
	   if(shares[obj_id].isWritable)
	   {int ret1 = map_frame(myenv->env_page_directory,frameinfo,i,PERM_WRITEABLE|PERM_USER);

	   }else
	   {   int ret1 = map_frame(myenv->env_page_directory,frameinfo,i,PERM_USER);
	   }
	//   if(ret1 == E_NO_MEM)
	//                {
	//        	       return E_NO_MEM;
